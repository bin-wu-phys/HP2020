...
\vspace{2mm}
\begin{enumerate}
    \item {\large Attractor in Stage I: classical statistical approximation (CSA):}
\begin{center}
    {\tiny  {\color{teablue}
...
\vspace{1mm}
    \item {\large Method:}\\
    \vspace{1mm}
    Including vacuum fluctuation in the initial condition {\color{darkred}$\Rightarrow f_{eq} = \frac{T}{p}-\frac{1}{2}$}.
...
\vspace{2mm}
    ...
    \vspace{2mm}
    \begin{itemize}
        \item{Leading-logs and resummation}
        \begin{align}
...
%\phantom
\begin{enumerate}
\item{\large{\bf There are some qualitative similarities to QCD} (left figure).}\\
\vspace{2mm}
\begin{itemize}
...
\end{itemize}
\vspace{1mm}
\item{{\bf The early-time dynamics may be modified by opacity} (right figure).}
\vspace{2mm}
\begin{center}
    {\bf\large \color{darkred} Potential probe to early-time dynamics!}
\end{center}
\end{enumerate}
\end{frame}
%
...
\end{center}
\vspace{4mm}
{\Large \color{black} in terms of  ''fluid quality''} for $T^{\mu\nu}$ calculated in some model
\begin{center}
{\Large\color{black}
%\framebox[1.1\width]
{
$Q(t,r) =  \sqrt{  \frac{ \left( T- T_{\rm hyd} \right)^{\mu\nu}   \left( T - T_{\rm hyd} \right)_{\mu\nu} }{ \left(T_{\rm id} \right)^{\mu\nu}    \left(T_{\rm id} \right)_{\mu\nu}    }  }.$
}
}
\vspace{2mm}\\
